  ...
  `,
    `
        ...
        vec3 rgbShift(sampler2D textureImage,vec2 uv,vec2 offset){
            float r=texture2D(textureImage,uv+offset*.1).r * 0.6;
...
const Service = (props) => {
    ...
    const textures = useTexture(['./img/loader/1.webp', './img/loader/2.webp', './img/loader/3.webp', './img/loader/4.webp'])
    ...
    useFrame((state) => {
        ...
        gsap.to(planeRef.current.position, {
            x: (mouse.x - (window.innerWidth / 2)) / 500,
            y: (-mouse.y + (window.innerHeight / 2)) / 500 + 1
        })
